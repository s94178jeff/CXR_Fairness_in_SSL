...
class NetWrapper(nn.Module):
    ...
    def _find_layer(self):
        if type(self.layer) == str:
            modules = dict([*self.encoder_q.named_modules()])
            return modules.get(self.layer, None)
        elif type(self.layer) == int:
            children = [*self.encoder_q.children()]
...
class BYOL(nn.Module):
    def __init__(
        ...
        self.forward(torch.randn(2, 1, image_size, image_size), torch.randn(2, 1, image_size, image_size))
